...
namespace RepairRequestMN
{
    internal class Sql : SqlProvider
    {
        ...
        /// <returns>Таблица со 1 строкой с полной информации о заявк</returns>
        public DataTable getSingleRepairRequest(int id)
        {
            ParameterValues.Clear();
            ParameterValues.Add(id);

            return executeProcedure("[Repair].[getSingleRepairRequest]", new[] { "@id" }, new[] { DbType.Int32 }, ParameterValues);
        }
        ...
        public DataTable GetComments(int id)
        {
            ParameterValues.Clear();
            ParameterValues.Add(id);
            return executeProcedure("[Repair].[GetCommentsById]", new[] { "@id" }, new[] { DbType.Int32 }, ParameterValues);
        }
    }
}